...
function getJson($opt){ // Handles CGI arguments
    $data = array();
    if(ISSET($opt['all']) and count($opt) === 2){ // Check if 'all' CGI is selected and if there is no more CGI except apikey
    ...
    elseif(ISSET($opt['all']) and count($opt) !== 2){ // if 'all' selected but more CGI
